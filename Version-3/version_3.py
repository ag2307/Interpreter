	...
	def __init__(self,string):
		...
		for i in self.statements:
			...
			if words[0]=="if":
				...
				function_count=1
			all_count=if_count|while_count|function_count
...
#			self.statements[i]=self.statements
		...
		self.all_statements=self.statements	
#		print(self.statements)
	...
	def eval(self,state):
		...
		global return_token
		for j,i in enumerate(self.statements):
			words=self.all_statements[j].strip().split()
			if words[0]=="function":
				function_token=i
				continue
			if words[0]=="return":
...
class Statement:
	def build(string):
		...
		if word[0]=="function":
			return FunctionClass(string)
		elif word[0]=="return":
			return ReturnStatement(string)
		elif word[0]=="if":
...
class FunctionClass:
	def __init__(self,string):
		self.function_name=re.findall(r'function(.*?)\(',string,re.DOTALL)[0].strip()
		self.arguments=re.findall(r'\((.*?)\)',string)[0]
		self.arguments=self.arguments.split(",")
		self.program=Program(re.findall(r'begin(.*?)end',string,re.DOTALL)[0].strip())
		self.state1={}
	def eval(state,name,arguments_call):
		if self.function_name==name:
			for i,j in enumerate(arguments_call):
				self.state1[self.arguments[i]]=Expression.build(j).eval(state)
			program.eval(self.state1)
			return self.state1
		else:
			raise NameError("Function name incorrect !!")
		

class FunctionCall:
	def __init__(self,string):
		self.function_name=string.split("(")[0].strip()
		self.arguments=re.findall(r'\((.*?)\)',string)[0]
		self.arguments=self.arguments.split(",")
	def eval(self,state):
		global function_token	
		global return_token	
		state1=function_token.eval(state,self.function_name,self.arguments)
		return return_token.eval(state1)
